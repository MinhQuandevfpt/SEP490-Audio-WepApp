...
} from "../../services/customer/ChatService";
...
const AIChatbot: React.FC = () => {
  ...
  const [conversations, setConversations] = useState<
    ConversationWithStoreInfo[]
  >([]);
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [selectedStore, setSelectedStore] =
    useState<ConversationWithStoreInfo | null>(null);
  ...
  const formatLastMessage = useCallback(
    (message: any): string => {
      ...
      if (message.messageType === "MIXED") {
        const mediaArray = Array.isArray(message.mediaUrl)
          ? message.mediaUrl
          : [];
        ...
        if (firstType === "image") {
          return mediaArray.length === 1
            ? "[Hình ảnh]"
            : `[${mediaArray.length} hình ảnh]`;
        } else {
          return mediaArray.length === 1
            ? "[Video]"
            : `[${mediaArray.length} video]`;
        }
      }
  ...
  useEffect(() => {
              ...
              if (c.storeId === conv.storeId) {
                ...
                if (newTime > currentTime) {
                  ...
                  return {
                    ...
                    customerUnreadCount: isSelected
                      ? 0
    ...
    chatMode,
    isAuthenticated,
    conversations.map((c) => c.storeId).join(","),
    formatLastMessage,
  ]);
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          const loadedMessages: Message[] = response.data.map((msg) => ({
            ...
            content: msg.content || "",
            messageType: (msg.messageType || "TEXT") as
              | "TEXT"
              | "IMAGE"
              | "VIDEO"
              | "MIXED",
          ...
          Promise.all([
            ...
            FirestoreChatService.updateMessagesReadStatus(
              customerId,
              selectedStore.storeId,
              "STORE"
            ),
    ...
    const unsubscribe = FirestoreChatService.subscribeToMessages(
      ...
      (firebaseMessages) => {
        ...
        if (firebaseMessages.length === 0) {
          ...
          const formattedMessages: Message[] = firebaseMessages.map(
            (msg): Message => {
              const role: "user" | "assistant" =
                msg.senderType === "CUSTOMER" ? "user" : "assistant";
    ...
    try {
      ...
      const conversationsWithStoreInfo = await Promise.all(
        convList.map(async (conv) => {
          try {
            const storeDetail = await CustomerStoreService.getStoreById(
              conv.storeId
            );
            ...
            return {
              ...conv,
              storeName:
                storeDetail.storeName || `Shop ${conv.storeId.substring(0, 8)}`,
              storeAvatar:
                storeDetail.logoUrl ||
                CustomerStoreService.getDefaultAvatar(storeDetail.storeName),
            ...
            return {
              ...
              storeAvatar: CustomerStoreService.getDefaultAvatar(
                `Shop ${conv.storeId.substring(0, 8)}`
              ),
            };
          }
        })
      ...
      const selectedStoreId = selectedStoreIdRef.current;
      const conversationsWithPreservedUnread = conversationsWithStoreInfo.map(
        (conv) => {
      ...
      if (targetConv) {
        ...
        try {
          ...
          const newConv: ConversationWithStoreInfo = {
            ...
            storeName:
              storeDetail.storeName || `Shop ${storeId.substring(0, 8)}`,
            storeAvatar:
              storeDetail.logoUrl ||
              CustomerStoreService.getDefaultAvatar(storeDetail.storeName),
          };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      if (chatMode === "ai") {
          ...
          try {
            ...
            for (const fileItem of filesToSend) {
              ...
              if (fileItem.type === "image") {
                const uploadResponse = await FileUploadService.uploadImage(
                  fileItem.file
                );
                uploadedUrl = uploadResponse.url;
              } else {
                const uploadResponse = await FileUploadService.uploadVideo(
                  fileItem.file
                );
                uploadedUrl = uploadResponse.url;
              }
            ...
            alert(
              uploadError.message || "Không thể tải file lên. Vui lòng thử lại."
            );
        ...
        const detectMediaTypeForMessage = (
          mediaItem: any
        ): "image" | "video" => {
          // Check if type field exists and is valid
          if (mediaItem?.type && typeof mediaItem.type === "string") {
        ...
        const formatLastMessageText = (): string => {
          ...
          if (messageType === "MIXED") {
            ...
            // Detect all media types in the array
            const mediaTypes = mediaArray.map((item) =>
              detectMediaTypeForMessage(item)
            );
            const hasImage = mediaTypes.includes("image");
            const hasVideo = mediaTypes.includes("video");
  ...
  const handleFileSelect = (
    ...
    if (chatMode === "ai") {
      alert(
        `Chỉ có thể gửi ${
          fileType === "image" ? "ảnh" : "video"
        } khi chat với cửa hàng`
      );
      return;
    }
      ...
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <span className="block text-[11px] text-white/80 leading-none">
                      Tư vấn tức thì
                    </span>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <span className="block text-[11px] text-orange-500/80 leading-none">
          ...
          >
            ...
            <span className="text-xs font-semibold whitespace-nowrap drop-shadow-sm">
              Chat Ngay
            </span>
          </button>
        </div>
                    ...
                    >
                      ...
                      {conv.storeAvatar ? (
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.src =
                              CustomerStoreService.getDefaultAvatar(
                                conv.storeName
                              );
                          }}
                        />
                        ...
                        <div className="max-w-[300px] min-w-0 space-y-2">
                          ...
                          {(() => {
                            ...
                            if (isMixed || isArray) {
                              ...
                              const isVideo =
                                message.messageType === "VIDEO" ||
                                mediaUrlString.match(/\.(mp4|webm|ogg)$/i);
