...
import { ChevronDown, CreditCard, DollarSign, Banknote, Landmark } from 'lucide-react';
...
const PaymentMethodDropdown: React.FC<Props> = ({ value, onChange }) => {
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow">
            {items.map(i => (
              <button key={i.key} onClick={() => { onChange(i.key); setOpen(false); }} className="w-full text-left px-3 py-2 hover:bg-gray-50 flex items-center gap-2">
                {i.icon}
