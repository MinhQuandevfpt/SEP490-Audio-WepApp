...
const PaymentMethodDropdown: React.FC<Props> = ({ value, onChange, disabled }) => {
  ...
  const items: { key: PaymentMethod; label: string; icon: React.ReactNode }[] = [
    { key: 'cod', label: t('checkout.payment.cod'), icon: <Banknote className="w-4 h-4" /> },
    { key: 'payos', label: t('checkout.payment.payos'), icon: <Smartphone className="w-4 h-4" /> },
  ];
  
  const current = items.find(i => i.key === value);
  const text = current ? current.label : t('checkout.payment.notSelected');

  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-2">
      <h3 className="text-lg font-semibold text-gray-900">{t('checkout.payment.selectTitle')}</h3>
