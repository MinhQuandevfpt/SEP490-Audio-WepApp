...
const NearbyProducts: React.FC = () => {
  ...
  const itemsPerPage = 10; // Hiển thị 10 sản phẩm mỗi lần
  ...
  const fetchProducts = async (page: number, reset: boolean = false) => {
    try {
      ...
      // Gọi API nearby products - chỉ lấy sản phẩm ACTIVE
      const response = await ProductViewService.getNearbyProducts({
        page: page,
        size: itemsPerPage,
        status: 'ACTIVE',
        sortBy: 'name',
      ...
      // Nếu lỗi 400 (chưa có địa chỉ mặc định), không hiển thị error
      if (err?.status === 400) {
        setError('CUSTOMER_NO_ADDRESS');
      } else {
  ...
  // Không hiển thị nếu có lỗi (đặc biệt là lỗi 400 - chưa có địa chỉ)
  if (error === 'CUSTOMER_NO_ADDRESS') {
    return null;
  }
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Sản phẩm gần tôi</h2>
        </div>
      </div>
      {loading && products.length === 0 ? (
        ...
        <>
          ...
          {/* View All Button - Hiển thị khi đã có 10 sản phẩm */}
          {products.length >= 10 && (
          ...
          {!loading && products.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">Không có sản phẩm nào gần bạn</p>
            </div>
          )}
        </>
      )}
...
export default React.memo(NearbyProducts);
