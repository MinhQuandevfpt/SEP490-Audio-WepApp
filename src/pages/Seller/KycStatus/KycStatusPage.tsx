...
import { Store, Clock, XCircle, AlertCircle, RefreshCw, CheckCircle } from 'lucide-react';
...
const KycStatusPage: React.FC = () => {
  ...
  const [storeStatus, setStoreStatus] = useState<'INACTIVE' | 'PENDING' | 'REJECTED' | 'ACTIVE' | null>(null);
  ...
  const loadStoreInfo = async () => {
    try {
      ...
      setStoreStatus(currentStatus as 'INACTIVE' | 'PENDING' | 'REJECTED');
      setIsLoading(false);
    } catch (error) {
  ...
  // REJECTED Status - Bị từ chối, cho phép đăng ký lại
  ...
  // Fallback - không có status hoặc status không hợp lệ
  return null;
};
